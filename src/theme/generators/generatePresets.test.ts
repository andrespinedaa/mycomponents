...
describe("generateComponentPresets", () => {
  ...
  describe("preset con orientation — nivel slot", () => {
    it("genera [data-slot][data-slots][data-set][data-orientation]", () => {
      ...
      expect(result).toContain(
        `[data-slot="CardSection"][data-slots="media"][data-set="background"][data-orientation="vertical"]{--card-section-aspect-ratio:3/4;}`,
      );
      expect(result).toContain(
        `[data-slot="CardSection"][data-slots="media"][data-set="background"][data-orientation="horizontal"]{--card-section-aspect-ratio:16/9;}`,
      );
    });
  });
});
